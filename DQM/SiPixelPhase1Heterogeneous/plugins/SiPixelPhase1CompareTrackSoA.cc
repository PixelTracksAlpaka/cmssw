...
void SiPixelPhase1CompareTrackSoA::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (int32_t it = 0; it < maxTracksCPU; ++it) {
    ...
    int nHitsGPU = helper::nHits(tsoaGPU.view(), closestTkidx);
    ...
    float phiGPU = helper::phi(tsoaGPU.view(), closestTkidx);
    float zipGPU = helper::zip(tsoaGPU.view(), closestTkidx);
    float tipGPU = helper::tip(tsoaGPU.view(), closestTkidx);
