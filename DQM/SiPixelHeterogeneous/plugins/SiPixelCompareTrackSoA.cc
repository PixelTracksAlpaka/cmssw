...
void SiPixelCompareTrackSoA<T>::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (int32_t jt = 0; jt < maxTracksGPU; ++jt) {
    ...
    if (!(helper::pt(tsoaGPU.view(), jt) > 0.))
      continue;
    nTracksGPU++;
...
<<<<<<< HEAD
    float chi2CPU = helper::chi2(tsoaCPU.view(),it);
=======
>>>>>>> d24fd27ff57 (Checks and comments)
    int nHitsCPU = helper::nHits(tsoaCPU.view(), it);

    if (nHitsCPU == 0)
      break;  // this is a guard

    int8_t nLayersCPU = helper::nLayers(tsoaCPU.view(),it);
    float ptCPU = helper::pt(tsoaCPU.view(),it);
    float etaCPU = helper::eta(tsoaCPU.view(),it);
    ...
    for (auto gid : looseTrkidxGPU) {
      float etaGPU = helper::eta(tsoaGPU.view(), gid);
