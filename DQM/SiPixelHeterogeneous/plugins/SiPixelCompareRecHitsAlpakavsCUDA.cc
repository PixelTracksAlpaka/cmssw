...
template <typename T>
class SiPixelCompareRecHitsSoAAlpakavsCUDA : public DQMEDAnalyzer {
public:
  using HitsAlpaka = TrackerRecHitHost<T>;
  using HitsCUDA = TrackingRecHitSoAHost<T>;

  explicit SiPixelCompareRecHitsSoAAlpakavsCUDA(const edm::ParameterSet&);
  ~SiPixelCompareRecHitsSoAAlpakavsCUDA() override = default;
...
//
template <typename T>
SiPixelCompareRecHitsSoAAlpakavsCUDA<T>::SiPixelCompareRecHitsSoAAlpakavsCUDA(const edm::ParameterSet& iConfig)
    : geomToken_(esConsumes<TrackerGeometry, TrackerDigiGeometryRecord, edm::Transition::BeginRun>()),
      topoToken_(esConsumes<TrackerTopology, TrackerTopologyRcd, edm::Transition::BeginRun>()),
      tokenSoAHitsAlpaka_(consumes(iConfig.getParameter<edm::InputTag>("pixelHitsSrcAlpakaCPU"))),
      tokenSoAHitsCUDA_(consumes(iConfig.getParameter<edm::InputTag>("pixelHitsSrcCUDACPU"))),
...
//
template <typename T>
void SiPixelCompareRecHitsSoAAlpakavsCUDA<T>::dqmBeginRun(const edm::Run& iRun, const edm::EventSetup& iSetup) {
...
//
template <typename T>
void SiPixelCompareRecHitsSoAAlpakavsCUDA<T>::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (not rhsoaHandleAlpaka or not rhsoaHandleCUDA) {
    edm::LogWarning out("SiPixelCompareRecHitsSoAAlpakavsCUDA");
...
//
template <typename T>
void SiPixelCompareRecHitsSoAAlpakavsCUDA<T>::bookHistograms(DQMStore::IBooker& iBook,
                                                       edm::Run const& iRun,
...
template<typename T>
void SiPixelCompareRecHitsSoAAlpakavsCUDA<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  // monitorpixelRecHitsSoAAlpaka
  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("pixelHitsSrcAlpakaCPU", edm::InputTag("siPixelRecHitsPreSplittingAlpakaSerial"));
  desc.add<edm::InputTag>("pixelHitsSrcCUDACPU", edm::InputTag("siPixelRecHitsPreSplittingSoA@cpu"));
  desc.add<std::string>("topFolderName", "SiPixelHeterogeneous/PixelRecHitsAlpakavsCUDA");
  desc.add<double>("minD2cut", 0.0001);
  descriptions.addWithDefaultLabel(desc);
}

using SiPixelPhase1CompareRecHitsSoAAlpakavsCUDA = SiPixelCompareRecHitsSoAAlpakavsCUDA<pixelTopology::Phase1>;
using SiPixelPhase2CompareRecHitsSoAAlpakavsCUDA = SiPixelCompareRecHitsSoAAlpakavsCUDA<pixelTopology::Phase2>;
using SiPixelHIonPhase1CompareRecHitsSoAAlpakavsCUDA = SiPixelCompareRecHitsSoAAlpakavsCUDA<pixelTopology::HIonPhase1>;

#include "FWCore/Framework/interface/MakerMacros.h"
DEFINE_FWK_MODULE(SiPixelPhase1CompareRecHitsSoAAlpakavsCUDA);
DEFINE_FWK_MODULE(SiPixelPhase2CompareRecHitsSoAAlpakavsCUDA);
DEFINE_FWK_MODULE(SiPixelHIonPhase1CompareRecHitsSoAAlpakavsCUDA);
