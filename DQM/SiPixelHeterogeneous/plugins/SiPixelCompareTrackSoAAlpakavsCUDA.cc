// -*- C++ -*-
// Package:    SiPixelCompareTrackSoAAlpakavsCUDA
// Class:      SiPixelCompareTrackSoAAlpakavsCUDA
//
/**\class SiPixelCompareTrackSoAAlpakavsCUDA SiPixelCompareTrackSoAAlpakavsCUDA.cc
*/
//
...
template <typename T>
class SiPixelCompareTrackSoAAlpakavsCUDA : public DQMEDAnalyzer {
public:
  using PixelTrackSoAAlpaka = PixelTrackHost<T>;
  using PixelTrackSoACUDA = TrackSoAHeterogeneousHost<T>;

  explicit SiPixelCompareTrackSoAAlpakavsCUDA(const edm::ParameterSet&);
  ~SiPixelCompareTrackSoAAlpakavsCUDA() override = default;
...
template <typename T>
SiPixelCompareTrackSoAAlpakavsCUDA<T>::SiPixelCompareTrackSoAAlpakavsCUDA(const edm::ParameterSet& iConfig)
...
//
template <typename T>
void SiPixelCompareTrackSoAAlpakavsCUDA<T>::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using helperAlpaka = PixelTrackUtilities<T>;
  ...
  if (not tsoaHandleAlpaka or not tsoaHandleCUDA) {
    edm::LogWarning out("SiPixelCompareTrackSoAAlpakavsCUDA");
  ...
  for (int32_t jt = 0; jt < maxTracksCUDA; ++jt) {
    ...
    if (!(tsoaCUDA.view()[jt].pt() > 0.))
      continue;
    nTracksCUDA++;
    if (useQualityCut_ && (pixelTrackSoA::Quality) qualityCUDA[jt] < minQuality_)
      continue;
  ...
  for (int32_t it = 0; it < maxTracksAlpaka; ++it) {
    ...
    float phiAlpaka = helperAlpaka::phi(tsoaAlpaka.view(), it);
    float zipAlpaka = helperAlpaka::zip(tsoaAlpaka.view(), it);
    float tipAlpaka = helperAlpaka::tip(tsoaAlpaka.view(), it);
    ...
    hCharge_->Fill(helperAlpaka::charge(tsoaAlpaka.view(), it), helperCUDA::charge(tsoaCUDA.view(), closestTkidx));
    ...
    hCurvdiffMatched_->Fill((helperAlpaka::charge(tsoaAlpaka.view(), it) / tsoaAlpaka.view()[it].pt()) -
...
//
template <typename T>
void SiPixelCompareTrackSoAAlpakavsCUDA<T>::bookHistograms(DQMStore::IBooker& iBook,
                                                     edm::Run const& iRun,
...
template<typename T>
void SiPixelCompareTrackSoAAlpakavsCUDA<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
using SiPixelPhase1CompareTrackSoAAlpakavsCUDA = SiPixelCompareTrackSoAAlpakavsCUDA<pixelTopology::Phase1>;
using SiPixelPhase2CompareTrackSoAAlpakavsCUDA = SiPixelCompareTrackSoAAlpakavsCUDA<pixelTopology::Phase2>;
using SiPixelHIonPhase1CompareTrackSoAAlpakavsCUDA = SiPixelCompareTrackSoAAlpakavsCUDA<pixelTopology::HIonPhase1>;

DEFINE_FWK_MODULE(SiPixelPhase1CompareTrackSoAAlpakavsCUDA);
DEFINE_FWK_MODULE(SiPixelPhase2CompareTrackSoAAlpakavsCUDA);
DEFINE_FWK_MODULE(SiPixelHIonPhase1CompareTrackSoAAlpakavsCUDA);
