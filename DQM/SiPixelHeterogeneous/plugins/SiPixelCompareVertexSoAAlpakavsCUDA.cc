// -*- C++ -*-
// Package:    SiPixelCompareVertexSoAAlpakavsCUDA
// Class:      SiPixelCompareVertexSoAAlpakavsCUDA
//
/**\class SiPixelCompareVertexSoAAlpakavsCUDA SiPixelCompareVertexSoAAlpakavsCUDA.cc
*/
//
...
class SiPixelCompareVertexSoAAlpakavsCUDA : public DQMEDAnalyzer {
public:
  using IndToEdm = std::vector<uint16_t>;
  explicit SiPixelCompareVertexSoAAlpakavsCUDA(const edm::ParameterSet&);
  ~SiPixelCompareVertexSoAAlpakavsCUDA() override = default;
  void bookHistograms(DQMStore::IBooker& ibooker, edm::Run const& iRun, edm::EventSetup const& iSetup) override;
  void analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) override;
  static void fillDescriptions(edm::ConfigurationDescriptions& descriptions);

private:
  const edm::EDGetTokenT<Vertex1DHost> tokenSoAVertexAlpaka_;
...
SiPixelCompareVertexSoAAlpakavsCUDA::SiPixelCompareVertexSoAAlpakavsCUDA(const edm::ParameterSet& iConfig)
    : tokenSoAVertexAlpaka_(consumes<Vertex1DHost>(iConfig.getParameter<edm::InputTag>("pixelVertexSrcAlpaka"))),
...
// -- Analyze
//
void SiPixelCompareVertexSoAAlpakavsCUDA::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (not vsoaHandleAlpaka or not vsoaHandleCUDA) {
    edm::LogWarning out("SiPixelCompareVertexSoAAlpakavsCUDA");
  ...
  if (!bsHandle.isValid()) {
    edm::LogWarning("SiPixelCompareVertexSoAAlpakavsCUDA") << "No beamspot found. returning vertexes with (0,0,Z) ";
  } else {
...
// -- Book Histograms
//
void SiPixelCompareVertexSoAAlpakavsCUDA::bookHistograms(DQMStore::IBooker& ibooker,
                                                   edm::Run const& iRun,
...
DEFINE_FWK_MODULE(SiPixelCompareVertexSoAAlpakavsCUDA);
