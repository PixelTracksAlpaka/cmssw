...
class PixelVertexProducerFromCUDASoA : public edm::global::EDProducer<> {
public:
  using IndToEdm = std::vector<uint32_t>;

  explicit PixelVertexProducerFromCUDASoA(const edm::ParameterSet &iConfig);
  ~PixelVertexProducerFromCUDASoA() override = default;
...
PixelVertexProducerFromCUDASoA::PixelVertexProducerFromCUDASoA(const edm::ParameterSet &conf)
...
void PixelVertexProducerFromCUDASoA::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
...
void PixelVertexProducerFromCUDASoA::produce(edm::StreamID streamID, edm::Event &iEvent, const edm::EventSetup &) const {
...
DEFINE_FWK_MODULE(PixelVertexProducerFromCUDASoA);
