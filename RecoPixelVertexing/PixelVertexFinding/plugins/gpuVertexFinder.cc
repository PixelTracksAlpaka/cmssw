...
namespace gpuVertexFinder {
  ...
  __global__ void loadTracks(TrackSoAConstView<TrackerTraits> tracks_view, VtxSoAView soa, WsSoAView pws, float ptMin, float ptMax) {
    assert(soa.idv());
...
template <typename TrackerTraits>
#ifdef __CUDACC__
  zVertex::ZVertexSoADevice Producer<TrackerTraits>::makeAsync(cudaStream_t stream,
...
#else
  zVertex::ZVertexSoAHost Producer<TrackerTraits>::make(TrackSoAConstView<TrackerTraits> tracks_view, float ptMin, float ptMax) const {
#ifdef PIXVERTEX_DEBUG_PRODUCE
