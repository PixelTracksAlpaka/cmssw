...
template <typename TrackerTraits>
void CAHitNtupletGeneratorKernelsCPU<TrackerTraits>::buildDoublets(HitsConstView hh, int32_t offsetBPIX2, cudaStream_t stream) {
...
template <typename TrackerTraits>
void CAHitNtupletGeneratorKernelsCPU<TrackerTraits>::launchKernels(HitsConstView hh, TkSoAView tracks_view, cudaStream_t cudaStream) {
...
template <typename TrackerTraits>
void CAHitNtupletGeneratorKernelsCPU<TrackerTraits>::classifyTuples(HitsConstView hh, TkSoAView tracks_view, cudaStream_t cudaStream) {
  using namespace caHitNtupletGeneratorKernels;

  int32_t nhits = hh.metadata().size();

  // auto const *tracks_view = &tracks_view->hitIndices;
  // auto *quality_d = tracks_view->qualityData();
  ...
  static std::mutex lock;
  {
    ...
    kernel_print_found_ntuplets<TrackerTraits>(
        hh, tracks_view, this->device_hitToTuple_.get(), 0, 1000000, iev);
  }
#endif
}
