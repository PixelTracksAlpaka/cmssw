...
#include "RecoPixelVertexing/PixelTrackFitting/interface/alpaka/BrokenLine.h"
...
//        ALPAKA_ASSERT_OFFLOAD(int(tkid) < foundNtuplets->nOnes());
  ...
  template <typename TrackerTraits>
  void HelixFitOnGPU<TrackerTraits>::launchBrokenLineKernels(const TrackingRecHitAlpakaSoAConstView<TrackerTraits> &hv,
                                                             uint32_t hitsInFit,
    ...
    for (uint32_t offset = 0; offset < maxNumberOfTuples; offset += maxNumberOfConcurrentFits_) {
      ...
      if (fitNas4_) {
        ...
        riemannFit::rolling_fits<4, TrackerTraits::maxHitsOnTrack, 1>(
            [this, &hv, &tkidGPU, &hitsGPU, &hits_geGPU, &fast_fit_resultsGPU, &offset, &queue, &workDivQuadsPenta](
                auto i) {
              alpaka::exec<Acc1D>(queue,
                                  workDivQuadsPenta,
