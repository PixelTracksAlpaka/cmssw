...
void CAHitNtupletGeneratorKernelsGPU<TrackerTraits>::launchKernels(HitsConstView hh,
  ...
  cms::cuda::finalizeBulk<<<numberOfBlocks, blockSize, 0, cudaStream>>>(this->device_hitTuple_apc_, &tracks_view.hitIndices()); //TODO test .data()
  ...
  cudaCheck(cudaGetLastError());
#endif
  kernel_fillNLayers<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(tracks_view, this->device_hitTuple_apc_);
  cudaCheck(cudaGetLastError());
...
template <typename TrackerTraits>
void CAHitNtupletGeneratorKernelsGPU<TrackerTraits>::buildDoublets(HitsConstView hh, int32_t offsetBPIX2, cudaStream_t stream) {
  int32_t nhits = hh.metadata().size();
...
template <typename TrackerTraits>
void CAHitNtupletGeneratorKernelsGPU<TrackerTraits>::classifyTuples(HitsConstView hh, TkSoAView tracks_view,
                                                                    cudaStream_t cudaStream) {
  using namespace caHitNtupletGeneratorKernels;
  ...
  static std::mutex lock;
  {
    ...
    for (int k = 0; k < 20000; k += 500) {
      kernel_print_found_ntuplets<TrackerTraits><<<1, 32, 0, cudaStream>>>(
          hh, tracks_view, this->device_hitToTuple_.get(), k, k + 500, iev);
      cudaDeviceSynchronize();
    }
    kernel_print_found_ntuplets<TrackerTraits><<<1, 32, 0, cudaStream>>>(
        hh, tracks_view, this->device_hitToTuple_.get(), 20000, 1000000, iev);
    cudaDeviceSynchronize();
