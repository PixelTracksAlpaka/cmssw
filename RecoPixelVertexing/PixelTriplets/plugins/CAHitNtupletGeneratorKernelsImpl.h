...
// printf("\n");
  ...
  __global__ void kernel_fishboneCleaner(GPUCACellT<TrackerTraits> const *cells,
    ...
    constexpr auto reject = pixelTrackSoA::Quality::dup;
  ...
  __global__ void kernel_earlyDuplicateRemover(GPUCACellT<TrackerTraits> const *cells,
    ...
    constexpr auto reject = pixelTrackSoA::Quality::edup;  /// cannot be loose
  ...
  __global__ void kernel_fastDuplicateRemover(GPUCACellT<TrackerTraits> const *__restrict__ cells,
                                              ...
                                              bool dupPassThrough) {
    // quality to mark rejected
    auto const reject = dupPassThrough ? pixelTrackSoA::Quality::loose : pixelTrackSoA::Quality::dup;
    constexpr auto loose = pixelTrackSoA::Quality::loose;
    ...
    for (int idx = first, nt = (*nCells); idx < nt; idx += gridDim.x * blockDim.x) {
      ...
      if (doit) {
        ...
        thisCell.template find_ntuplets<maxDepth>(
            hh, cells, *cellTracks, tracks_view.hitIndices(), *apc, tracks_view.quality(), stack, params.minHitsPerNtuplet_, bpix1Start);
  ...
  __global__ void kernel_countMultiplicity(TkSoAView<TrackerTraits> tracks_view,
    ...
    for (int it = first, nt = tracks_view.hitIndices().nOnes(); it < nt; it += gridDim.x * blockDim.x) {
      ...
      if (nhits < 3)
        continue;
      if (tracks_view[it].quality() == pixelTrackSoA::Quality::edup)
        continue;
      assert(tracks_view[it].quality() == pixelTrackSoA::Quality::bad);
  ...
  template <typename TrackerTraits>
  __global__ void kernel_classifyTracks(TkSoAView<TrackerTraits> tracks_view,
    ...
    for (int it = first, nt = tracks_view.hitIndices().nOnes(); it < nt; it += gridDim.x * blockDim.x) {
      ...
      tracks_view[it].quality() = pixelTrackSoA::Quality::strict;

      if (cuts.strictCut(tracks_view, it))
        continue;

      tracks_view[it].quality() = pixelTrackSoA::Quality::tight;

      if (cuts.isHP(tracks_view, nhits, it))
        tracks_view[it].quality() = pixelTrackSoA::Quality::highPurity;
    }
  }

  template <typename TrackerTraits>
  __global__ void kernel_doStatsForTracks(TkSoAView<TrackerTraits> tracks_view,
                                          Counters *counters) {
    int first = blockDim.x * blockIdx.x + threadIdx.x;
    for (int idx = first, ntot = tracks_view.hitIndices().nOnes(); idx < ntot; idx += gridDim.x * blockDim.x) {
     if (tracks_view.hitIndices().size(idx) == 0)
        break;  //guard
      if (tracks_view[idx].quality() < pixelTrackSoA::Quality::loose)
        continue;
      atomicAdd(&(counters->nLooseTracks), 1);
      if (tracks_view[idx].quality() < pixelTrackSoA::Quality::strict)
        continue;
  ...
  template <typename TrackerTraits>
  __global__ void kernel_countHitInTracks(TkSoAView<TrackerTraits> tracks_view,
  ...
  template <typename TrackerTraits>
  __global__ void kernel_fillHitInTracks(TkSoAView<TrackerTraits> tracks_view,
                                         HitToTuple<TrackerTraits> *hitToTuple) {
    int first = blockDim.x * blockIdx.x + threadIdx.x;
    for (int idx = first, ntot = tracks_view.hitIndices().nOnes(); idx < ntot; idx += gridDim.x * blockDim.x) {
      if (tracks_view.hitIndices().size(idx) == 0)
  ...
  template <typename TrackerTraits>
  __global__ void kernel_fillHitDetIndices(TkSoAView<TrackerTraits> tracks_view,
                                           HitsConstView<TrackerTraits> hh) {
    int first = blockDim.x * blockIdx.x + threadIdx.x;
    // copy offsets
 ...
 */
  ...
  __global__ void kernel_markSharedHit(int const *__restrict__ nshared,
                                       ...
                                       Quality *__restrict__ quality,
                                       bool dupPassThrough) {
    // constexpr auto bad = pixelTrackSoA::Quality::bad;
    constexpr auto dup = pixelTrackSoA::Quality::dup;
    constexpr auto loose = pixelTrackSoA::Quality::loose;
    // constexpr auto strict = pixelTrackSoA::Quality::strict;
  ...
  __global__ void kernel_sharedHitCleaner(HitsConstView<TrackerTraits> hh,
    ...
    auto const longTqual = pixelTrackSoA::Quality::highPurity;
  ...
  __global__ void kernel_tripletCleaner(TkSoAView<TrackerTraits> tracks_view,
                                        ...
                                        HitToTuple<TrackerTraits> const *__restrict__ phitToTuple) {
    // quality to mark rejected
    auto const reject = pixelTrackSoA::Quality::loose;
    /// min quality of good
    auto const good = pixelTrackSoA::Quality::strict;
  ...
  __global__ void kernel_simpleTripletCleaner(TkSoAView<TrackerTraits> tracks_view,
    ...
    auto const reject = pixelTrackSoA::Quality::loose;
    /// min quality of good
    auto const good = pixelTrackSoA::Quality::loose;
    ...
    for (int idx = first, ntot = hitToTuple.nOnes(); idx < ntot; idx += gridDim.x * blockDim.x) {
      ...
      for (auto ip = hitToTuple.begin(idx); ip != hitToTuple.end(idx); ++ip) {
        auto const it = *ip;
        if (tracks_view[it].quality() > reject && tracksUtilities<TrackerTraits>::isTriplet(tracks_view, it) && it != im)
          tracks_view[it].quality() = reject;  //no race:  simple assignment of the same constant
      }
