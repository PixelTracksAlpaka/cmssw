...
struct TracksUtilities {
  ...
  constexpr static float charge(const TrackSoAConstView &tracks, int32_t i) {
    //return std::copysign(1.f, tracks[i].state()(2));
    auto v = tracks[i].state()(2);
  ...
  constexpr static float tip(const TrackSoAConstView &tracks, int32_t i) {
    return tracks[i].state()(1);
  }

  constexpr static float zip(const TrackSoAConstView &tracks, int32_t i) {
    return tracks[i].state()(4);
  }

  constexpr static bool isTriplet(const TrackSoAConstView &tracks, int i) {
    return tracks[i].nLayers() == 3;
  }

  template <typename V3, typename M3, typename V2, typename M2>
  constexpr static void copyFromCircle(
  ...
  constexpr static void copyFromDense(TrackSoAView &tracks,
                                                                          V5 const &v,
                                                                          M5 const &cov,
                                                                          int32_t i) {
  ...
  constexpr static int computeNumberOfLayers(const TrackSoAConstView &tracks,
                                                                                  int32_t i) {
...
namespace pixelTrack {
  ...
  struct QualityCutsT<TrackerTraits, pixelTopology::isPhase1Topology<TrackerTraits>> {
    ...
    constexpr bool isHP(const TrackSoAConstView &tracks, int nHits, int it) const {
    ...
    constexpr bool strictCut(const TrackSoAConstView &tracks, int it) const {
